...
function main()
  ...
  if houseRules then
    ...
    Rule.eval("$a={};$a.b=77")
    local vv = ScriptCompiler.test("daily({10:00,11:00})")
    print(json.encode(vv))
    --Rule.new("{type='property', deviceID=55} => off(66)")
    _setClock("t/08:00")
    _setMaxTime(48)
-- Kitchen
...
-- Living room
    ...
    Rule.new("daily(00:00) => off($lr.lamp_window);log('Turn off livingroom light')")
...
-- Tim
    ...
    Rule.new("daily(00:00) => off({$ti.bed_led,$ti.lamp_window});log('Turn off lights for Tim')")

-- Max
    Rule.new("daily(sunset-00:10) => on($ma.lamp_window);log('Turn on lights for Max')")
    Rule.new("daily(00:00) => off($ma.lamp_window);log('Turn off lights for Max')")
...
-- Bathroom
    ...
    Rule.new("for(00:10,safe($td.movement)&value($td.door)) => not($inBathroom)&off($td.lamp_roof)")
    Rule.new("breached($td.movement) => if(safe($td.door),$inBathroom=true);on($td.lamp_roof)")
    Rule.new("breached($td.door) => $inBathroom=false")
    Rule.new("safe($td.door)&(last($td.movement)<3) => $inBathroom=true")

-- SceneActivation events
    Rule.new([[scene($lr.lamp_roof_holk)==$S2.click =>
  ...
  if ruleTests then
    ...
    Rule.new("daily(10:00)&day('1-7')&wday('mon') => log('10 oclock first Monday of the month!')")
    Rule.new("daily(10:00)&day('lastw-last')&wday('mon') => log('10 oclock last Monday of the month!')")
  ...
  local function getTriggers(e)
    ...
    local function gt(k,e)
      ...
      elseif tFun[k] then local v = ScriptEngine.eval(ScriptCompiler.compile(e[2]))
        Util.map(function(id) ids[id]={type='property', deviceID=id} end,type(v)=='table' and v or {v})
      end
      return e
    end
    traverse(e,gt)
    return ids and Util.mapkl(function(k,v) return v end,ids),dailys
  end

  function self.test(s)
    return {getTriggers(self.parse(s))}
  end
...
function newRuleCompiler()
  ...
  function self.new(expro)
    ...
    elseif type(p) == 'table' then
      ...
      if next(env.time) then -- 'daily' rule
      ...
      elseif next(env.id) or next(env.glob) then -- id/glob trigger rule
