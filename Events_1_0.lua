...
function QuickApp:main()
  ...
  local s = Event.loop("00:01",function(r) Log(LOG.LOG,"X:%s",r.count) end).disable() -- loop every minute. start disabled
  Event.post(s.enable,"+/00:02")  -- enable loop after 2min
  Event.post(s.disable,"+/00:10") -- disable loop after 10min
...
function createEventEngine(quickSelf) -- Event extension
  ...
  local equal,format,map,mapF,copy =  Utils.equal, string.format, Utils.map, Utils.mapF, Utils.copy
  ...
  self.isTimer, self.isRule, self.isEvent, self.mkTimer = isTimer, isRule, isTimer, mkTimer
  ...
  function self.cron(pattern,fun)
    local test,c = Utils.dateTest(pattern),0
...
function createUtils(quickSelf)
  ...
  self.equal,self.copy = equal,copy
  ...
  LOG = { LOG="[L]", SYS="[Sys]", DEBUG="[D]", ERROR='[ERROR]', HEADER='HEADER'}
...
function createAutoPatchSupport(quickSelf)
  ...
  function Utils.checkVersion(vers)
  ...
  function Utils.patchEventRunner(newSrc)
    if newSrc == nil then
      local req = net.HTTPClient()
