...
function main()
  ...
  sensorIDs = {201,203,202}  -- Sensor IDs, need to decalare these in the header
  luxIDs = {401,403,402}  -- Sensor IDs, need to decalare these in the header
  lightIDs = {129,130}       -- Light IDs
  luxLightIDs = {131,132}    -- Light IDs depending on lux
  luxLimit = 250

  Rule.eval("for(00:03,sensorIDs:safe) => lightIDs:off; luxLightIDs:off")
  Rule.eval("sensorIDs:breached => lightIDs:on; sum(luxIDs:lux)/length(luxIDs) < luxLimit & luxLightIDs:on")

  Rule.eval("sensorIDs:off")
  Rule.eval("luxIDs:value=300")
  Rule.eval("wait(+/00:10); 201:value=1")
end -- main()
------------------- EventModel --------------------  
