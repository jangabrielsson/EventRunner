...
-- Bedroom
  ...
  if true then
    ...
    function post(e,t) t = t or osTime(); Event.post(e,t) end
    function prop(id,state,prop) return {type='property', deviceID=id, propertyName=prop, value=state} end
    function glob(id,state) return {type='global', name=id, value=state} end
    _setClock("a08:00")
  end
  if nil then
    Rule.new("daily(10) => 6,8")
    ...
    Rule.new("for(00:10,not(safe($hall.door))) => send($user.jan.phone,log('Door open %s min',repeat(5)*10))")

    Rule.new("for(00:15,safe($sensorsDownstairs)) => betw(00:00,05:00)&off($lampsDownstairs)")

    Rule.new("daily(sunset-00:45) => log('Sunset')&on($sunsetLamps)")
    Rule.new("daily(sunrise+00:15) => log('Sunrise')&off($sunriseLamps)")

    Rule.new("!homeStatus=='away' => post(#simulate{action='start'})")
    Rule.new("!homeStatus=='home' => post(#simulate{action='stop'})")

    Rule.new("#simulate{action='start'} => log('Starting simulation')")
    Rule.new("#simulate{action='stop'} => log('Stopping simulation')")
    ...
    Rule.new("scene($kitchen.switch)==$S1.click => log('S1 switch clicked')")

    post(prop(d.kitchen.switch,Util.S1.click,'sceneActivation'),"n/09:10")

    Rule.new("csEvent(56).keyId=='4' => log('HELLO key=4')")
    Rule.new("weather() => log('HELLO %s',weather().newValue)")

    post({type='event',event={type='CentralSceneEvent',data={deviceId=56, keyId='4',keyAttribute='Pressed'}}},"n08:10")
    post({type='event',event = {type="WeatherChangedEvent", data = {newValue=-2.2,change="Temperature"}}},"n08:20")

    Rule.eval("{b=2,c={d=3}}.c.d") 

    Rule.macro('LAST31',"(date('jan,mar,may,jul,aug,oct,dec')&date('25-31'))")
    Rule.macro('LAST30',"(date('apr,jun,sep,nov')&date('24-30'))")
    Rule.macro('LAST28',"(date('feb')&date('22-28'))")
    Rule.new("daily(10:00)&date('1-7')&date('mon') => log('10 oclock first Monday of the month!')")
    Rule.new("daily(10:00)&($LAST31$|$LAST30$|$LAST28$)&date('mon') => log('10 oclock last Monday of the month!')")
...
function newEventEngine()
  ...
  function self.post(e,time) -- time in 'toTime' format, see below.
    _assert(isEvent(e) or type(e) == 'function', "Bad2 event format %s",Util.prettyJson(e))
  ...
  function self.event(e,action) -- define rules - event template + action
    ...
    local rule,fn = {[self.RULE]=e, action=action, org=Util.prettyJson(args)}, true
  ...
  local function _invokeRule(env)
    ...
    if not status then
      ...
      self.post({type='error',err=res,rule=env.rule.org,event=Util.prettyJson(env.event),_sh=true})    -- Send error back
...
function newScriptEngine()
  ...
  instr['for'] = function(s,n,a,e,i) 
    ...
    elseif (not i[5]) and val then                        -- Timer not running, and true, start timer
      i[5]=Event.post(function() i[6] = true; i[5] = nil; 
          self.eval(e.o,e) 
        end,time+osTime()) 
    end
    s.push(false)
  ...
  function self.continue(o)
    ...
    if status then return res
      ...
      if not instr[i[1]] then errThrow("eval",_format("undefined function '%s'",i[1])) end
...
function newScriptCompiler()
  ...
  _comp['quote'] = function(e,ops) ops[#ops+1] = {mkOp('const'),0,e[2]} end
...
function newRuleCompiler()
  ...
  local mtr={isOn=1,isOff=1,power=1,bat=1,lux=1,safe=1,breached=1,sense=1,manual=1,
    value=1,temp=1,scene=1,['for']=2,once=2,daily=2,glob=2,['.']=2}
  ...
  function self.addTrigger(name,code,ftr)
  ...
  function self.eval(expr, log)
    ...
    if log then Log(LOG.LOG,"%s = %s",expr,Util.prettyJson(res)) end
    return res
  end
...
local function _dateInstr(s,n,a,e,i)
  local ts = s.pop()
  if ts ~= i[5] then i[6] = Util.dateTest(ts); i[5] = ts end -- cache fun
  s.push(i[6]())
end
ScriptEngine.addInstr("date",_dateInstr)
ScriptEngine.addInstr("day",_dateInstr)
ScriptEngine.addInstr("hour",_dateInstr)
...
Rule.addTrigger('weather',
  ...
  function(s,n,a,e) if n>0 then e.id[s.pop()]=mkW else e.id['*']=function() return {type='WeatherChangedEvent'} end end s.push(true) end)
