...
_sceneName = "Time and lights" -- Set to scene/script name
_debugLevel   = 3
...
--]]  Event.event({type='start'},    function(env)--      Event.schedule("n/00:10",{type='daily_init'}) -- Run daily setup 10min past midnight--      if now() > "00:10" then Event.post({type='daily_init'}) end -- Run at startup  end)  Event.event({type='daily_init'},     function(env)--      Log(LOG_COLOR,"Sunrise at %s",fibaro:getValue(1,"sunriseHour"))--      Log(LOG_COLOR,"Sunset at %s",fibaro:getValue(1,"sunsetHour"))--      Event.post({type='Sunrise'},"t/sunrise")--      Event.post({type='Sunset'},"t/sunset")--      local d = {'sun','mon','tue','wed','thu','fri','sat'}--      local t = os.date("*t",osTime())--      local ne,evs = nil,sched[d [t.wday ] ]--      table.sort(evs,function(a,b) return a[1] < b[1] end)--      for _,e in ipairs(evs) do--        if past(e[1 ]) then ne = e[2 ] else Event.post({type=e[2 ]},"t/"..e[1]) end--      end--      if ne then Event.post({type=ne}) end --latest 'past' event posted - i.e. to set time of day
      ...
      Rule.eval("td.door:safe & td.movement:last<3 => inBathroom=true")-- Kitchen  Rule.eval("for('00:10',k.movement:safe & k.lamp_table:isOn) => wday('mon-fr')&hour('8-12')|hour('0-4')&k.lamp_table:off& log('Turning off kitchen lamp after 10 min inactivity')")  Rule.eval("for('00:10',{k.movement,l.movement,h.movement}:safe & {k.lamp_stove,k.lamp_sink,h.lamp_hall}:isOn) => \z
      wday('mon-fr') & hour('8-12') | hour('0-4') & {k.lamp_stove,k.lamp_sink,h.lamp_hall}:off & \z
      log('Turning off kitchen spots after 5 min inactivity')"
    )-- Kitchen
    Rule.eval("#kitchen{value='on'} => k.sink.led:btn=1 ; log('Turn on kitchen sink light')")
    Rule.eval("#kitchen{value='off'} => k.sink.led:btn=2 ; log('Turn off kitchen sink light')")  Rule.eval("#kitchen{value='on'} => k.lamp_table:on; log('Evening, turn on kitchen table light')")-- Living room  Rule.eval("#livingroom{value='on'} => l.lamp_window:on; log('Turn on livingroom light')")
  ...
  Rule.eval([[k.lamp_table:scene==S2.click => 
    || label(k.sonos,'lblState')=='Playing' >> k.sonos:btn=8 
    || true >> k.sonos:btn=8 ;;
    log('Toggling Sonos %s',label(k.sonos,'lblState'))]])  Rule.eval([[#property{deviceID=l.lamp_window} =>        || l.lamp_window:isOn >> l.lamp_tv:btn=1; l.lamp_globe:btn=1
    || true >> l.lamp_tv:btn=2; l.lamp_globe:btn=2 ;;
...
function newScriptEngine()
  ...
  getIdFun['safe']=getIdFun['off'] getIdFun['breached']=getIdFun['on']
  ...
  instr['setProp'] = function(s,n,e,i) local id,v=s.pop(),s.pop() 
    if setIdFun[i[3]] then setIdFun[i[3]](s,i,id,v) else setIdFuns(s,i,i[3],id,v) end; s.push(v) 
  end
