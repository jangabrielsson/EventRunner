...
function main()
  ...
  -- lets start
  if nil then 
    Rule.eval("post(#Sunset,t/sunset-00:10)")

    scheduler = { scheds={} }
    function scheduler:add(args) 
      if type(args.action) ~= 'function' then
        local action = type(args.action[1])~='table' and {args.action} or args.action
        args.action = function() for _,a in ipairs(action) do 
          _ENV.fibaro[a[1]](select(1,table.unpack(a))) end end
        end
        self.scheds[#self.scheds+1]=args 
      end
      function scheduler:toTime(time)
        if time:match("sun") then time = fibaro:getValue(1,time.."Hour") end
        h,m,s = time:match("(%d%d):(%d%d):?(%d*)")
        return (s=="" and 0 or s)+60*(m+h*60) 
      end
      function scheduler:midnight() local t = osDate("*t"); return {year=t.year,month=t.month,day=t.day,hour=0,min=0,sec=0} end
      function scheduler:run() 
        local mid,now,last = midnight(),osTime(),midnight()
        for _,e in ipairs(self.scheds) do
          local time = mid+toTime(e.time)
          if time < now and e.catchup then e.action()
          elseif time >= now then setTimeout(e.action,1000*(time-now)) end
        end
        local function loop()
          local mid,now = midnight(),osTime()
          for _,e in ipairs(self.scheds) do setTimeout(e.action,1000*(mid+toTime(e.time)-now)) end
          last=last+24*60*60
          setTimeout(loop,1000*(last-now))
        end
        last = last+24*60*60
        setTimeout(loop,1000*(last-now))
      end

      local timeStr = "%Y:%m:%d:%H:%M:%S"
      local midnight = "%Y:%m:%d:00:00:00"

      function pd(t) print(string.format("Time:%s",osDate("%c",t))) end
      ...
      local now = osTime()
      pd(now)
      pd(str2time(osDate(timeStr)))

      local s = "8:9:10:11"
      s = s:gsub("(%d+:%d+):(%d+):(%d+)",function(a,b,c) return string.format("%s:%s:%s",a,b+1,c) end)
      print(s)
      ...
      --scheduler:add{time="10:00",action={{'call',77,'turnOn'},{'call',55,'turnOn'}}}
    end
    --scheduler:run()
