...
function main()
  ...
  for i,j in ipairs({
      ...
      --"a={}; a.b=7; a.b",
      --"log('hello %d',a.b)",
      --"a=ostime();dotimes(v,1,10000,v*10);log('Time:%ss',(ostime()-a))",
      --"isOn(66) => log('SUM:%d',(fn(a,b)-> return(a+b) end)(7,8))",
...
--      "room={lamp=44}; room.lamp:on",
  ...
  y=Rule.eval("a=ostime();dotimes(v,1,10000,v*10);log('Time:%ss',(ostime()-a))")
  printRule(y)
...
function newScriptEngine()
  ...
  local devFun={}
  devFun['on']=function(id) return fibaro:getValue(id,'value')>'0' end
  devFun['off']=function(id) return fibaro:getValue(id,'value')=='0' end
  ...
  instr['%rule'] = function(s,n) local r,b,h=s.pop(),s.pop(),s.pop() s.push(Rule.compRule({'=>',h,b},r)) end
  instr['%prop'] = function(s,n,f,e,i) s.push(devFun[f](ID(s.pop(),i))) end
  instr['apply'] = function(s,n,f,e) local f = s.pop()
  ...
  instr['setRef'] = function(s,n,a) local r,v = s.pop(),s.pop() -- setRef <table>, <value>, [,key]
    if n==3 then r,a=s.pop(),r end
  ...
  instr['aref'] = function(s,n,a,e,i) local k,tab = s.pop() -- aref <tab> [,<key>]
    if n==2 then tab=s.pop() else tab,k=k,a end
...
function newScriptCompiler()
  ...
  local function compT(e,ops)
    if type(e) == 'table' then
      ...
      else for i=2,#e do compT(e[ i],ops) end ops[#ops+1] = {mkOp(e[ 1]),#e-1} end
    else 
  ...
  _comp['%prop'] = function(e,ops) _assert(isVar(e[3]),"bad property field: '%s'",e[3])
    compT(e[2],ops) ops[#ops+1]={mkOp('%prop'),1,e[3][2]} 
  end
...
--  preC['>>'] = function(k,e) return self.precompile({'and',e[2],{'always',e[3]}}) end -- test >> expr |||| test >> expr ||| t >> expr
  ...
  local _tokens = {
    ...
    {"^(%()",'lpar'},{"^(%))",'rpar'},
    {"^([;,])",'token'},{"^(if)",'token'},{"^(then)",'token'},{"^(else)",'token'},{"^(end)",'token'},
...
function newRuleCompiler()
  ...
  function self.compRule(e,r)
    ...
    if type(h)=='table' and (h[1]=='table' or h[1]=='quote' and type(h[2])=='table') then -- event matching rule, Needs check for 'type'!!!!
      ...
      res = Event.event((ScriptEngine.eval(ep)),body)
      local ll = res.action
